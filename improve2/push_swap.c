...
/*   Updated: 2023/05/02 17:50:15 by mamagalh@st      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ps_moves	*ft_pair_attach(t_ps_moves *node, int *nb_a, int *nb_b)
{
	...
	{
		while (temp_lst->stacks[stack][0] != nb_a[2])
		{
			ft_next_node(&temp_lst, &ft_rotate, nb_a[0]);
			temp_lst = ft_dblstlast(temp_lst);
		}
		while (temp_lst->stacks[stack][0] != nb_b[2])
		{
			ft_next_node(&temp_lst, &ft_rotate, nb_b[0]);
			temp_lst = ft_dblstlast(temp_lst);
		}
...
int (*ft_pair_next(int **stacks, int *stack, int *end)) (int **, int, int *, int)
{
	//pass
}
